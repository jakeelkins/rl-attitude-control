...
class AttitudeControlEnv(gym.Env):
    ...
    def __init__(self, torque_scale = 0.5, steps = 500, frameskip = 20):

        # multiply this to scale the torques. we'll loop thru training to change this
        self.torque_scale = torque_scale

        self.frameskip_num = frameskip #controls agent fideltiy. number of times to integrate between actions
        ...
        self.I = np.array([0.872, 0.115, 0.797])    #rotational inertia tensor (assuming symmetric)
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            # -1 in x
            torque = np.array([-0.01, 0, 0])
        elif action == 2:
            # +1 in x
            torque = np.array([0.01, 0, 0])
        elif action == 3:
            # -0.00001 in y
            torque = np.array([0, -0.01, 0])
        elif action == 4:
            # 0.00001 in y
            torque = np.array([0, 0.01, 0])
        elif action == 5:
            # -1 in z
            torque = np.array([0, 0, -0.01])
        elif action == 6:
            # +1 in z
            torque = np.array([0, 0, 0.01])
        elif action == 7:
        ...
        torque = self.torque_scale * torque

        #step the env once with the torque applied:
        #self.omega, self.q_ref = self._RK4_omega_quat(self.h, self.omega, self.q_ref, torque, self.I)
        ...
        q_error_val = np.dot(self.alpha, np.square(self.q_error))
